...
# dialogue after defeating skeleton
with open(r"/Users/ashleylim/PycharmProjects/NEA project/dialogue_text.txt", 'r') as fp:
    # lines to read
    line_numbers = [35, 36]
    # To store lines
    skeleton_defeat_dialogue = []
    for i, line in enumerate(fp):
        if i in line_numbers:
            skeleton_defeat_dialogue.append(line.strip())
...
class Character(pygame.sprite.Sprite):
    def __init__(self, char_type, x, y, scale, max_hp, strength, heal):
        ...
        # reset my list of images
        #player_idle = "/Users/ashleylim/PycharmProjects/NEA project/character animation/player/idle"
...
enemy = Character("Skeleton", 750, 180, 4, 30, 7, 1)
...
while running:
    ...
    #draw_text(f"Strength: {player.strength}", font, (255, 255, 255), 140, PHEIGHT - bottom_panel - 80)
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_e:
                ...
                speed = 3
                active_message = 0
                message = skeleton_defeat_dialogue[active_message]
                ...
                while run:
                    ...
                    for event in pygame.event.get():
                        if event.type == pygame.QUIT:
                            pygame.quit()
                            sys.exit()
                        if event.type == pygame.MOUSEBUTTONDOWN and active_message < 1 or event.type == pygame.KEYDOWN and event.key == pygame.K_RETURN:
                            active_message += 1
                            message = skeleton_defeat_dialogue[active_message]
                            counter = 0
                            ...
                            if active_message == 1:
                                message = skeleton_defeat_dialogue[active_message]
                                import enemy_encounters.enemy_encounter3
